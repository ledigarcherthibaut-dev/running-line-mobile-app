...
export function OnboardingStepLayout({
  ...
  const { tokens, resolvedScheme, setMode } = useTheme();
      ...
      <ScrollView contentContainerStyle={styles.container}>
        <View style={styles.topRow}>
          ...
          <Pressable
            ...
            style={[styles.themeToggle, { backgroundColor: tokens.surface2, borderColor: tokens.border2 }]}
            accessibilityRole="button"
            accessibilityLabel={isDark ? 'Passer au thème clair' : 'Passer au thème sombre'}
          >
            <Icon name={isDark ? 'moon' : 'sun'} size={16} color={tokens.text2} />
          </Pressable>
        </View>
...
const styles = StyleSheet.create({
  ...
  progressRow: { flex: 1, gap: 8 },
  progressLabel: { fontSize: 10.5, letterSpacing: 1 },
  progressTrack: { height: 4, borderRadius: 2, overflow: 'hidden' },
  progressFill: { height: 4, borderRadius: 2 },
  themeToggle: { width: 34, height: 34, borderRadius: radii.full, alignItems: 'center', justifyContent: 'center', borderWidth: 1 },
